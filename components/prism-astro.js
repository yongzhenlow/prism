/**
 * @see https://github.com/withastro/astro/blob/main/packages/astro-prism/src/plugin.ts
 */
(function (Prism) {
	...
	var script = Prism.util.clone(Prism.languages[scriptLang]);
	var space = /(?:\s|\/\/.*(?!.)|\/\*(?:[^*]|\*(?!\/))\*\/)/.source;
	var braces = /(?:\{(?:\{(?:\{[^{}]*\}|[^{}])*\}|[^{}])*\})/.source;
	...
	Prism.languages.astro.tag.pattern = re(/<\/?(?:[\w.:-]+(?:<S>+(?:[\w.:$-]+(?:=(?:"(?:\\[\s\S]|[^\\"])*"|'(?:\\[\s\S]|[^\\'])*'|[^\s{'"/>=]+|<BRACES>))?|<SPREAD>))*<S>*\/?)?>/
		.source);
	Prism.languages.astro.tag.inside['tag'].pattern = /^<\/?[^\s>\/]*/;
	Prism.languages.astro.tag.inside['attr-value'].pattern =
        /=(?!\{)(?:"(?:\\[\s\S]|[^\\"])*"|'(?:\\[\s\S]|[^\\'])*'|[^\s'">]+)/;
	Prism.languages.astro.tag.inside['tag'].inside['class-name'] =
        /^[A-Z]\w*(?:\.[A-Z]\w*)*$/;
